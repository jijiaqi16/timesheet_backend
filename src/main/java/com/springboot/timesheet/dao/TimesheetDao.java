...
public interface TimesheetDao extends JpaRepository<Timesheet, Long> {
    ...
    @Query(value =
            "select ts.id from timesheet ts\n" +
                    "join timesheet_employee te on ts.id=te.timesheet_id\n" +
                    "join employee e on te.emp_id=e.id\n" +
                    " where (ts.timesheet_start_date=? && e.emp_username=?);", nativeQuery = true)
    List<Long> findIdsByStartDate(Date startDate,String username);
    ...
    @Transactional
    void removeTimesheetById(Long id);
}
